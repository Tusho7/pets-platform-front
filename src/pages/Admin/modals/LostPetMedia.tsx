import { LostPetMediaProps } from "../../../types/Admin/MediaProps";
import { useTranslation } from "react-i18next";

const LostPetMedia: React.FC<LostPetMediaProps> = ({
  selectedPet,
  closeModal,
}) => {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
      <div className="bg-white p-4 rounded-lg shadow-lg max-w-lg w-full">
        <h2 className="text-xl font-semibold mb-4">
          {t("lostPetMedia.mediaTitle", { petName: selectedPet.pet_name })}
        </h2>

        {selectedPet?.images && selectedPet.images.length > 0 ? (
          <div className="mb-4">
            <h3 className="font-medium mb-2">
              {t("lostPetMedia.imagesTitle")}
            </h3>
            <div className="overflow-x-auto flex space-x-2 max-h-48">
              ...
              {selectedPet.images.length > 3 && (
                <button className="px-2 py-1 bg-blue-500 text-white rounded">
                  {t("lostPetMedia.showMore")}
                </button>
              )}
            </div>
          </div>
        ) : (
          <p>{t("lostPetMedia.noImages")}</p>
        )}

        {selectedPet?.videos && selectedPet.videos.length > 0 ? (
          <div className="mb-4">
            <h3 className="font-medium mb-2">
              {t("lostPetMedia.videosTitle")}
            </h3>
